...
2026-03-18 PATCH:
- Delta tab now supports optional minimum repeat-size filtering
- User can filter out repeat sizes below a chosen cutoff
- Delta is then computed only on the remaining repeat sizes and frequencies

2026-04-01 PATCH:
- instability_results.csv now includes Individual_Mode
- This is the mode detected from each individual sample histogram
- Forced_Mode remains the mode actually used for instability calculation

2026-04-15 PATCH:
- Delta tab now supports optional manual Control/D0 mode reindexing
- User can enter a Control/D0 mode of choice for bin offset alignment
- If entered, that value is used as offset 0 instead of auto-detected control mode

2026-04-17 PATCH:
- Delta tab now has SEPARATE manual Control/D0 offset options for Dataset 1 and Dataset 2
- Dataset 1 and Dataset 2 can each use their own user-entered Control/D0 mode as offset 0
- Delta tab CSV outputs now use user-provided Dataset 1 / Dataset 2 names in filenames
- CSV outputs now include the analysis timestamp in YYMMDD_HHMM format
- Final realigned / KS comparison CSVs now concatenate BOTH dataset names in the filename
  Example:
    260417_1027_PMS1_D42vD0_normalized_with_offset.csv
    260417_1027_PMS2_D42_vD0_normalized_with_offset.csv
    260417_1027_PMS1_D42vD0__PMS2_D42_vD0_realigned_zero_filled_deltas.csv

2026-05-19 PATCH:
- Replaced large-number wx.SpinCtrl inputs with wx.TextCtrl plus validation
- Prevents packaged GUI builds from blocking long numeric values before run-time

2026-05-13 PATCH:
- Removed hard coded words for control and whatever folder used as controls those mode will be used for bin offset
-Forced mode option in delta tab.   
-increased Histogram limit

2026-06-29 PATCH:
- Delta tab now exposes the per-dataset minimum repeat-size filter in the GUI
- Separate checkbox + value for Dataset 1 and Dataset 2
- Entering e.g. 30 excludes all repeat sizes below 30 (keeps 30 and up) before
  normalization, so excluded sizes do not contribute to column sums or the delta
"""
...
def find_docker():
    """Locate the docker executable on this host.

    The original code hardcoded /usr/local/bin/docker, which only exists on
    macOS/Linux. On Windows that raises WinError 2. Resolve via PATH first,
    then fall back to the usual install locations for each platform.
    """
...
# Stop Windows flashing a console window for every docker call once the GUI
# is built with console=False.
_SUBPROC_FLAGS = {}
if platform.system() == "Windows":
    _SUBPROC_FLAGS["creationflags"] = getattr(subprocess, "CREATE_NO_WINDOW", 0)
...
# -----------------------------------
def make_numeric_text_ctrl(parent, value, size=(120, -1)):
    """Create a plain text field for numbers.

    wx.SpinCtrl is convenient for small numbers, but it enforces a fixed min/max
    and can prevent users from typing long values in packaged GUI builds.
    Plain TextCtrl + validation lets Python handle large integers safely.
    """
    return wx.TextCtrl(parent, value=str(value), size=size, style=wx.TE_PROCESS_ENTER)


def _clean_numeric_text(ctrl, label):
    raw = str(ctrl.GetValue()).strip()
    if not raw:
        raise ValueError(f"Please enter {label}.")
    # Allow friendly thousands separators such as 1,000,000.
    return raw.replace(",", "")


def get_int_from_ctrl(ctrl, label, min_value=None, max_value=None):
    raw = _clean_numeric_text(ctrl, label)
    try:
        value = int(raw)
    except ValueError:
        raise ValueError(f"{label} must be a whole number. You entered: {ctrl.GetValue()}")
...
def get_float_from_ctrl(ctrl, label, min_value=None, max_value=None):
    raw = _clean_numeric_text(ctrl, label)
    try:
        value = float(raw)
    except ValueError:
        raise ValueError(f"{label} must be a number. You entered: {ctrl.GetValue()}")
...
# -----------------------------
# Instability runner with 3 strategies (PATCHED for rest/per)
# -----------------------------
def run_instability_analysis(
    ...
    for hist_path in hist_files:
        ...
        individual_mode, _ = detect_mode_robust(df)

        if mode_strategy == "per_sample":
            used_mode = int(individual_mode)
        else:
        ...
        results.append(
            {
                ...
                "Individual_Mode": int(individual_mode),
...
class GenerateHistogramsPanel(wx.Panel):
    def __init__(self, parent):
        ...
        self.manual_mode_spin = make_numeric_text_ctrl(self, "0", size=(140, -1))
        ...
        self.repeat_size_spin = make_numeric_text_ctrl(self, "250", size=(150, -1))
        ...
        self.threshold_spin = make_numeric_text_ctrl(self, "5.0", size=(120, -1))
    ...
    def on_run(self, _):
        ...
        try:
            threshold_percent = get_float_from_ctrl(
                self.threshold_spin,
                "Threshold % for filtering",
                min_value=0.1,
                max_value=100.0,
            )
            repeat_size = get_int_from_ctrl(
                self.repeat_size_spin,
                "Repeat Size for Histogram",
                min_value=50,
            )
        ...
        if self.inst_mode_normal_rb.GetValue():
            ...
            try:
                manual_mode = get_int_from_ctrl(self.manual_mode_spin, "Manual forced mode", min_value=1)
            except ValueError as e:
                wx.MessageBox(str(e), "Input error", wx.OK | wx.ICON_ERROR)
                return
    ...
    def _run_all(
        ...
        try:
            ...
            if tar_file and os.path.isfile(tar_file) and tar_file.endswith(".tar"):
                if not DOCKER_EXE:
                    self.log(
                        "Error: Docker executable not found. Install Docker Desktop "
                        "and make sure it is running and on your PATH."
                    )
                    wx.CallAfter(self._processing_complete, False, None)
                    return
                self.log(f"Loading Docker image from {tar_file}...")
                load_cmd = [DOCKER_EXE, "load", "-i", tar_file]
                result = subprocess.run(
                    load_cmd, capture_output=True, text=True, **_SUBPROC_FLAGS
                )
            ...
            hist_ext = hist_suffix_for_profile(profile)
            ...
            for i, fasta_file in enumerate(all_fasta_files, 1):
                ...
                # Argument list rather than a shell string: avoids quoting
                # problems with Windows paths containing spaces/backslashes.
                # /usr/local/bin/RepeatDetecter is the path INSIDE the Linux
                # container and must stay as-is.
                docker_cmd = [
                    DOCKER_EXE, "run", "--rm",
                    "-v", f"{fasta_dir}:/mnt/fasta",
                    "-e", "LD_LIBRARY_PATH=/app/RepeatDetector/build/external/htslib/src/htslib",
                    "repeat-detector",
                    "/usr/local/bin/RepeatDetecter",
                    "--prf", prf_file,
                    f"/mnt/fasta/{os.path.basename(fasta_file)}",
                    "--output-name", f"/mnt/fasta/{out_name}",
                    "-o", "histogram",
                    "--with-revcomp",
                    "--cycle-range", f"[0:{repeat_size}]",
                    "--verbose",
                ]
                result = subprocess.run(
                    docker_cmd,
...
def align_to_global_mode(df, control_cols=None, forced_mode=None):
    bin_col = df.columns[0]

    if forced_mode is not None:
        global_mode = forced_mode
    else:
        if control_cols is None or len(control_cols) == 0:
            ...
            if not control_cols:
                control_cols = list(sample_cols[:1])
...
def filter_histogram_df_by_min_repeat(df, min_repeat_size):
    if df is None or df.empty:
        return df

    out = df.copy()
    repeat_col = out.columns[0]
    out[repeat_col] = pd.to_numeric(out[repeat_col], errors="coerce")
    out = out.dropna(subset=[repeat_col])
    out = out[out[repeat_col] >= min_repeat_size].copy()
...
class RepeatDetectorPanel(wx.Panel):
    def __init__(self, parent):
        ...
        self.xlim_left_ctrl = make_numeric_text_ctrl(self, "-100", size=(120, -1))
        ...
        self.xlim_right_ctrl = make_numeric_text_ctrl(self, "100", size=(120, -1))
        ...
        self.bin_size_ctrl = make_numeric_text_ctrl(self, "1", size=(100, -1))
        settings_row1.Add(self.bin_size_ctrl, 0)
        plot_sizer.Add(settings_row1, 0, wx.EXPAND | wx.ALL, 5)

        reindex_row1 = wx.BoxSizer(wx.HORIZONTAL)
        self.enable_manual_mode_reindex_cb1 = wx.CheckBox(
            self, label="Dataset 1: Enter Control/D0 mode to use as offset 0:"
        )
        self.enable_manual_mode_reindex_cb1.SetValue(False)
        reindex_row1.Add(self.enable_manual_mode_reindex_cb1, 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 8)

        self.manual_mode_reindex_spin1 = make_numeric_text_ctrl(self, "0", size=(140, -1))
        reindex_row1.Add(self.manual_mode_reindex_spin1, 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 8)

        reindex_row1.Add(wx.StaticText(self, label="(used only for Dataset 1)"), 0, wx.ALIGN_CENTER_VERTICAL)
        plot_sizer.Add(reindex_row1, 0, wx.EXPAND | wx.ALL, 5)

        reindex_row2 = wx.BoxSizer(wx.HORIZONTAL)
        self.enable_manual_mode_reindex_cb2 = wx.CheckBox(
            self, label="Dataset 2: Enter Control/D0 mode to use as offset 0:"
        )
        self.enable_manual_mode_reindex_cb2.SetValue(False)
        reindex_row2.Add(self.enable_manual_mode_reindex_cb2, 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 8)

        self.manual_mode_reindex_spin2 = make_numeric_text_ctrl(self, "0", size=(140, -1))
        reindex_row2.Add(self.manual_mode_reindex_spin2, 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 8)

        reindex_row2.Add(wx.StaticText(self, label="(used only for Dataset 2)"), 0, wx.ALIGN_CENTER_VERTICAL)
        plot_sizer.Add(reindex_row2, 0, wx.EXPAND | wx.ALL, 5)

        # -----------------------------------------------------------------
        # Minimum repeat-size filtering (per dataset) -- 2026-06-29 PATCH
        # Wires the existing filter_histogram_df_by_min_repeat() helper into
        # the GUI. Entering e.g. 30 excludes all repeat sizes below 30.
        # -----------------------------------------------------------------
        minrep_row1 = wx.BoxSizer(wx.HORIZONTAL)
        self.enable_min_repeat_cb1 = wx.CheckBox(
            self, label="Dataset 1: Exclude repeat sizes below:"
        )
        self.enable_min_repeat_cb1.SetValue(False)
        minrep_row1.Add(self.enable_min_repeat_cb1, 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 8)

        self.min_repeat_spin1 = make_numeric_text_ctrl(self, "0", size=(140, -1))
        minrep_row1.Add(self.min_repeat_spin1, 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 8)

        minrep_row1.Add(wx.StaticText(self, label="(used only for Dataset 1)"), 0, wx.ALIGN_CENTER_VERTICAL)
        plot_sizer.Add(minrep_row1, 0, wx.EXPAND | wx.ALL, 5)

        minrep_row2 = wx.BoxSizer(wx.HORIZONTAL)
        self.enable_min_repeat_cb2 = wx.CheckBox(
            self, label="Dataset 2: Exclude repeat sizes below:"
        )
        self.enable_min_repeat_cb2.SetValue(False)
        minrep_row2.Add(self.enable_min_repeat_cb2, 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 8)

        self.min_repeat_spin2 = make_numeric_text_ctrl(self, "0", size=(140, -1))
        minrep_row2.Add(self.min_repeat_spin2, 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 8)

        minrep_row2.Add(wx.StaticText(self, label="(used only for Dataset 2)"), 0, wx.ALIGN_CENTER_VERTICAL)
        plot_sizer.Add(minrep_row2, 0, wx.EXPAND | wx.ALL, 5)
    ...
    def on_clear_all(self, _):
        ...
        self.enable_manual_mode_reindex_cb1.SetValue(False)
        self.manual_mode_reindex_spin1.SetValue("0")
        self.enable_manual_mode_reindex_cb2.SetValue(False)
        self.manual_mode_reindex_spin2.SetValue("0")
        self.enable_min_repeat_cb1.SetValue(False)
        self.min_repeat_spin1.SetValue("0")
        self.enable_min_repeat_cb2.SetValue(False)
        self.min_repeat_spin2.SetValue("0")
        self.color1_choice.SetSelection(0)
        self.color2_choice.SetSelection(0)
    ...
    def on_run(self, _):
        try:
            ...
            safe_label1 = clean_label(legend_label1)
            safe_label2 = clean_label(legend_label2)
            analysis_stamp = current_analysis_stamp()

            dataset1_prefix = f"{analysis_stamp}_{safe_label1}"
            dataset2_prefix = f"{analysis_stamp}_{safe_label2}"
            combined_prefix = f"{analysis_stamp}_{safe_label1}__{safe_label2}"

            use_manual_mode_reindex1 = self.enable_manual_mode_reindex_cb1.GetValue()
            manual_reindex_mode1 = (
                get_int_from_ctrl(self.manual_mode_reindex_spin1, "Dataset 1 Control/D0 mode", min_value=1)
                if use_manual_mode_reindex1
                else None
            )

            use_manual_mode_reindex2 = self.enable_manual_mode_reindex_cb2.GetValue()
            manual_reindex_mode2 = (
                get_int_from_ctrl(self.manual_mode_reindex_spin2, "Dataset 2 Control/D0 mode", min_value=1)
                if use_manual_mode_reindex2
                else None
            )

            use_min_repeat1 = self.enable_min_repeat_cb1.GetValue()
            min_repeat_size1 = (
                get_int_from_ctrl(self.min_repeat_spin1, "Dataset 1 minimum repeat size", min_value=0)
                if use_min_repeat1
                else None
            )

            use_min_repeat2 = self.enable_min_repeat_cb2.GetValue()
            min_repeat_size2 = (
                get_int_from_ctrl(self.min_repeat_spin2, "Dataset 2 minimum repeat size", min_value=0)
                if use_min_repeat2
                else None
            )

            bin_size = get_int_from_ctrl(self.bin_size_ctrl, "Bin size", min_value=1)
            xlim_left = get_int_from_ctrl(self.xlim_left_ctrl, "X-axis left limit")
            xlim_right = get_int_from_ctrl(self.xlim_right_ctrl, "X-axis right limit")
            if xlim_left >= xlim_right:
                raise ValueError("X-axis left limit must be smaller than X-axis right limit.")
            ...
            if use_min_repeat1:
                df_control1 = filter_histogram_df_by_min_repeat(df_control1, min_repeat_size1)
                df_treatment1 = filter_histogram_df_by_min_repeat(df_treatment1, min_repeat_size1)

            merged1 = pd.merge(df_control1, df_treatment1, on="repeat length", how="outer").fillna(0)
            ...
            aligned_df1, global_mode1 = align_to_global_mode(stats_df1, control_cols=control_cols1, forced_mode=manual_reindex_mode1)

            norm_csv1 = os.path.join(out_dir, f"{dataset1_prefix}_normalized_with_offset.csv")
            aligned_df1.to_csv(norm_csv1, index=False)

            binned_data1 = bin_delta_by_offset_with_auc(aligned_df1, bin_size=bin_size)

            binned_csv1 = os.path.join(out_dir, f"{dataset1_prefix}_binned_by_offset.csv")
            ...
            basic_stats1["manual_reindex_mode_enabled"] = use_manual_mode_reindex1
            basic_stats1["manual_reindex_mode_value"] = manual_reindex_mode1 if use_manual_mode_reindex1 else ""
            basic_stats1["min_repeat_filter_enabled"] = use_min_repeat1
            basic_stats1["min_repeat_filter_value"] = min_repeat_size1 if use_min_repeat1 else ""
            basic_stats1["dataset_label"] = legend_label1
            basic_stats1["analysis_timestamp"] = analysis_stamp
            auc_stats1 = analyze_auc_stats(binned_data1)
            all_stats1 = {**basic_stats1, **auc_stats1}
            stats_csv1 = os.path.join(out_dir, f"{dataset1_prefix}_stats.csv")
            pd.DataFrame([all_stats1]).to_csv(stats_csv1, index=False)
            ...
            if self.enable_dataset2_cb.GetValue():
                ...
                if c2 and t2 and os.path.isdir(c2) and os.path.isdir(t2):
                    ...
                    if use_min_repeat2:
                        df_control2 = filter_histogram_df_by_min_repeat(df_control2, min_repeat_size2)
                        df_treatment2 = filter_histogram_df_by_min_repeat(df_treatment2, min_repeat_size2)

                    merged2 = pd.merge(df_control2, df_treatment2, on="repeat length", how="outer").fillna(0)
                    ...
                    aligned_df2, global_mode2 = align_to_global_mode(stats_df2, control_cols=control_cols2, forced_mode=manual_reindex_mode2)
                    norm_csv2 = os.path.join(out_dir, f"{dataset2_prefix}_normalized_with_offset.csv")
                    aligned_df2.to_csv(norm_csv2, index=False)

                    binned_data2 = bin_delta_by_offset_with_auc(aligned_df2, bin_size=bin_size)
                    binned_csv2 = os.path.join(out_dir, f"{dataset2_prefix}_binned_by_offset.csv")
                    ...
                    basic_stats2["manual_reindex_mode_enabled"] = use_manual_mode_reindex2
                    basic_stats2["manual_reindex_mode_value"] = manual_reindex_mode2 if use_manual_mode_reindex2 else ""
                    basic_stats2["min_repeat_filter_enabled"] = use_min_repeat2
                    basic_stats2["min_repeat_filter_value"] = min_repeat_size2 if use_min_repeat2 else ""
                    basic_stats2["dataset_label"] = legend_label2
                    basic_stats2["analysis_timestamp"] = analysis_stamp
                    auc_stats2 = analyze_auc_stats(binned_data2)
                    all_stats2 = {**basic_stats2, **auc_stats2}
                    stats_csv2 = os.path.join(out_dir, f"{dataset2_prefix}_stats.csv")
                    pd.DataFrame([all_stats2]).to_csv(stats_csv2, index=False)

            ks_offset_result = None
            aligned_offsets_csv = None
            ks_offsets_csv = None
            if self.enable_dataset2_cb.GetValue() and binned_data2 is not None and binned_csv2 is not None:
                aligned_offsets_csv, ks_offsets_csv, ks_offset_result = align_tailtrim_and_ks_on_offsets(
                    ...
                    name2=legend_label2,
                    tol=0.0,
                    out_aligned_csv=f"{combined_prefix}_realigned_zero_filled_deltas.csv",
                    out_ks_csv=f"{combined_prefix}_ks_results.csv",
                )
            ...
            ylim = None
            ymin_raw = self.ymin_ctrl.GetValue().strip()
            ymax_raw = self.ymax_ctrl.GetValue().strip()
            try:
                ymin = float(ymin_raw) if ymin_raw else None
                ymax = float(ymax_raw) if ymax_raw else None
            except ValueError:
                raise ValueError("Y-axis min and max must be valid numbers.")
            if ymin is not None and ymax is not None:
                if ymin >= ymax:
                    raise ValueError("Y-axis min must be smaller than Y-axis max.")
                ylim = (ymin, ymax)
            ...
            status_msg = f"Done. Saved: {png_path} | Timestamp={analysis_stamp} | Dataset1 CSV prefix={dataset1_prefix}"
            if self.enable_dataset2_cb.GetValue():
                status_msg += f" | Dataset2 CSV prefix={dataset2_prefix}"

            if use_manual_mode_reindex1:
                status_msg += f" | Dataset1 reindexed with Control/D0 mode={manual_reindex_mode1}"
            if use_manual_mode_reindex2:
                status_msg += f" | Dataset2 reindexed with Control/D0 mode={manual_reindex_mode2}"

            if use_min_repeat1:
                status_msg += f" | Dataset1 excluded repeat sizes < {min_repeat_size1}"
            if use_min_repeat2:
                status_msg += f" | Dataset2 excluded repeat sizes < {min_repeat_size2}"
