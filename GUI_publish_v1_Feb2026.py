...
def run_instability_analysis(
    ...
    hist_ext=(".rest.histogram", ".per.histogram"),  # PATCH
):
    """
...
class GenerateHistogramsPanel(wx.Panel):
    ...
    def _run_all(
        ...
        try:
            ...
            hist_ext = hist_suffix_for_profile(profile)  # PATCH: rest/per histogram suffix
            ...
            for i, fasta_file in enumerate(all_fasta_files, 1):
                ...
                if result.returncode != 0:
                    ...
                    hist_path = os.path.join(fasta_dir, f"{out_name}.histogram")  # sample.rest.histogram or sample.per.histogram
            ...
            if generate_enhanced_plots:
                ...
                # PATCH: use hist_ext (rest OR per)
                ...
                if d0_files:
                    for d0_fasta in d0_files:
                        ...
                        for hist_file in os.listdir(hist_dir):
                            # PATCH: match rest or per depending on chosen profile
...
def read_histogram_files_with_folder_prefix(file_paths, folder_path, group_type, use_thresholded=False):
    ...
    for path in file_paths:
        ...
        # PATCH: strip ".rest" / ".per" tags from the stem for nicer labels
        if is_thresholded:
...
# -----------------------------
# Delta GUI panel (unchanged except it benefits from label cleanup above)
# -----------------------------
